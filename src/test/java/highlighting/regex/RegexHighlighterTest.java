...
class RegexHighlighterTest {
  ...
  void simpleNoOverlap() {
    List<HighlightRegion> result =
        highlighter.resolveConflicts(highlighter.collectMatches("class Test"));
  ...
  @Test
  void emptyString() {
    List<HighlightRegion> result = highlighter.resolveConflicts(highlighter.collectMatches(""));
  ...
  void noMatches() {
    List<HighlightRegion> result =
        highlighter.resolveConflicts(highlighter.collectMatches("hello world"));
  ...
  void adjacentRegionsAreKept() {
    List<HighlightRegion> input =
        List.of(new HighlightRegion(0, 5, null), new HighlightRegion(5, 10, null));
  ...
  void keywordInsideCommentOverlap() {
    List<HighlightRegion> regions = highlighter.collectMatches("// class Test");
  ...
  void javadocAndBlockCommentOverlap() {
    ...
    List<HighlightRegion> regions = highlighter.collectMatches(text);
  ...
  void overlappingRegionsKeepFirst() {
    List<HighlightRegion> input =
        List.of(new HighlightRegion(0, 10, null), new HighlightRegion(5, 15, null));
